 ...
 * seconds.c
 *
 * Show the value of time elapsed in seconds between loading and unloading module by communicating through proc file system
...
MODULE_DESCRIPTION("PROC FILE FOR SECONDS ELAPSED"); /*This macro contains the description of the present working kernel code*/

#define BUFFER_SIZE 128 /*defines a buffer of size 128 bytes that is used by this program*/
#define PROC_NAME "seconds" /*This is where the module gets its name*/
...
ssize_t proc_read(struct file* file, char __user* usr_buf, size_t count, loff_t* pos)
{
    ...
    end_time = jiffies; /*Store the value of jiffies during reading the file*/

    rv = sprintf(buffer, "SECONDS: %lu\n", (end_time - start_time) / HZ); /*Value of seconds stored in jiffies variable that is a part of kernel buffer space*/
